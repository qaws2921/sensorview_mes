...
function basicGet(object,jqGrid_object,page){
	...
	if (object.condition_class !=null) {
		...
		if (effectiveness_condition(object,object.condition_objact)) {
			$(jqGrid_object.main_name).setGridParam({ url: object.getUrl ,datatype: "json", page: page,postData:object.condition_objact}).trigger("reloadGrid");			
		}
	}else {
		
		$(jqGrid_object.main_name).setGridParam({ url: object.getUrl ,datatype: "json", page: page}).trigger("reloadGrid");
	}
