...
function delete_btn() {
    ...
    if (ids.length === 0) {
        ...
        if (confirm("삭제하겠습니까?")) {
            main_data.check = 'D';
            wrapWindowByMask2();
            ccn_ajax("/popBcrFormDel", {keyword:keywords.join(gu5)}).then(function (data) {
                if (data.result === 'NG') {
                    alert(data.message);
                } else {
...
function jqGrid_main() {
    $('#mes_grid').jqGrid({
        ...
        colModel: [
            {name: 'bcr_form_code', index: 'bcr_form_code', key: true, sortable: false, width: 60},
            {name: 'bcr_form_name', index: 'bcr_form_name', sortable: false, width: 60},
