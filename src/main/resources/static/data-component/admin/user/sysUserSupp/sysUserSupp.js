...
function update_btn(jqgrid_data) {
	if (main_data.auth.check_edit !="N") {
		...
		ccn_ajax('/sysUserSuppOneGet', {keyword:jqgrid_data.user_code}).then(function (data) { // user의 하나 출력
	...
	if(main_data.auth.check_del != "N") {
		...
		if (ids.length === 0) {
			...
			if (confirm("삭제하겠습니까?")) {
				main_data.check = 'D';
				wrapWindowByMask2();
				ccn_ajax("/sysUserSuppDelete", {keyword: ids.join(gu5)}).then(function (data) {
					if (data.result === 'NG') {
						alert(data.message);
					} else {
...
////////////////////////////호출 함수/////////////////////////////////////
function authcheck() {
	ccn_ajax("/menuAuthGet", {keyword: "sysUserSupp"}).then(function (data) {
		main_data.auth = data;
	});
}
